...
export default function IntakePage() {
  ...
  async function submit() {
    ...
    try {
      ...
      setRisk(result);
      setMessage(
        "Intake submitted and assessment calculated. Review the result with the clinician.",
      );
    } catch (error) {
      setMessage(
  ...
  return (
    <main className="page-wrap intake-page">
      <div className="page-heading">
        <div>
          <p className="eyebrow">New patient assessment</p>
          <h1>Patient intake</h1>
          <p>
            Combine patient context, live dictation, vital signs, and verified
            laboratory evidence.
          </p>
        </div>
        {risk && (
          <div className="risk-result">
            <span>Arrival Risk Index</span>
            <strong>{risk.ari}</strong>
            <small>
              ESI {risk.esi} · {risk.confidence} confidence
            </small>
          </div>
        )}
      </div>
      <section className="intake-section">
        <div className="section-title">
          ...
          <div>
            ...
            <p>Demographics and identifiers from the patient or report.</p>
          </div>
        </div>
        <div className="field-grid four">
          ...
          <Field
            ...
            onChange={(value) => update("age_years", value)}
            type="number"
            placeholder="Years"
          />
          <Field
          ...
          <Field
            ...
            value={patientData.registration_no}
            onChange={(value) => update("registration_no", value)}
            placeholder="MRN"
          />
          <Field
            label="Referred by"
            ...
            placeholder="Clinician or service"
          />
          <Field
            label="Report date"
            value={patientData.report_date}
            onChange={(value) => update("report_date", value)}
      ...
      <section className="intake-section">
        <div className="section-title">
          <span>02</span>
          <div>
            <h2>Presentation and vitals</h2>
            <p>Capture the reason for arrival and observations at intake.</p>
          </div>
        </div>
        <div className="field-grid six">
          {(
            ...
            <label className="intake-field" key={key}>
              <span>
                {label} <em>{unit}</em>
              </span>
              <input
                type="number"
                value={patientData.vitals[key]}
        ...
        <div className="notes-grid">
          <div>
            <label className="textarea-label">Chief complaint</label>
            <textarea
          ...
          <div>
            <label className="textarea-label">Nursing assessment</label>
            <textarea
              value={patientData.nursing_assessment}
              onChange={(event) =>
                update("nursing_assessment", event.target.value)
              }
              placeholder="Observations, symptoms, and relevant context"
            />
            <VoiceIntake
              submitToBackend={false}
              onTranscript={(text) =>
                update(
                  "nursing_assessment",
                  patientData.nursing_assessment
                    ? `${patientData.nursing_assessment} ${text}`
                    : text,
                )
              }
            />
      ...
      <section className="intake-section">
        ...
        >
          ...
          <strong>
            {parsing
              ? "Parsing laboratory report..."
              : "Drop a PDF report here"}
          </strong>
          <span>or click to browse · PDF files only</span>
        </div>
        {patientData.test_results.length > 0 && (
          <div className="editable-results">
            <div className="editable-result editable-header" aria-hidden="true">
              <span>Test name</span>
              <span>Value</span>
              <span>Unit</span>
              <span>Reference range</span>
              <span>Abnormality flag</span>
              <span />
            </div>
            {patientData.test_results.map((result, index) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateResult(index, "reference_range", event.target.value)
                  }
                  placeholder="Reference range"
                />
                <input
                  aria-label="Abnormality flag"
                  value={result.abnormality_flag || ""}
                  onChange={(event) =>
                    updateResult(index, "abnormality_flag", event.target.value)
                  }
                  placeholder="H / L / A"
                  maxLength={1}
                />
                <button
                  ...
                  onClick={() =>
                    setPatientData((current) => ({
                      ...current,
                      test_results: current.test_results.filter(
        ...
        <div className="intake-status">{message}</div>
      </section>
      <div className="intake-submit">
        <div>
          <strong>Ready for clinical review?</strong>
          <span>
            The full reviewed payload will be sent to the ARI assessment
            endpoint.
          </span>
        </div>
        <button className="primary-action" type="button" onClick={submit}>
          Calculate Arrival Risk Index
        </button>
      </div>
    </main>
  );
